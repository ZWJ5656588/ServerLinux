...
{
	m_complete = false;
	memset(&m_parser, 0, sizeof(m_parser)); // 转换
	m_parser.data = this;
	// 收到客户端请求
...
{
	return ((CHttpParser*)(parser->data))->OnMessageBegin(); // 注意运算符有优先级
}
...
int UrlParser::Parser()
{
	...
	if (target == NULL) {
		...
		m_uri = Buffer(pos + 1, target); // 获取到uri 本服务器是login
		//解析key和value
		pos = target + 1;
		const char* t = NULL;
		do {
...
{
	auto it = m_values.find(name); // 键不在m_value字典中
